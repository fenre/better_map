...
| (managed regions inside `../reference/*.md` and `../integrations/catalogue.md`) | E2 Phase 2 — auto-generated tables that live INSIDE the hand-authored human-readable pages. Currently two managed regions: (a) the 83-option formatter enumeration table set in [`docs/reference/formatter.md`](https://github.com/fenre/better_map/blob/main/docs/reference/formatter.md) (grouped by Dashboard Studio tab → in-tab heading, with type, default, enum / range, and help text per option), and (b) the at-a-glance integrations matrix + endpoint detail subsection in [`docs/integrations/catalogue.md`](https://github.com/fenre/better_map/blob/main/docs/integrations/catalogue.md) (one row per integration with status, required Splunk app, version min, REST endpoint count, auth model, OT-safety flag, live-tenant test status, and a link back to the source YAML — plus a per-integration `#### <id>` bullet list of every endpoint the viz calls). The content lives between `<!-- BEGIN AUTOGEN: <section-id> -->` / `<!-- END AUTOGEN: <section-id> -->` marker pairs; hand-authored narrative outside the markers (including the eight per-integration `## <integration>` prose blocks in catalogue.md) is preserved verbatim. | `_machine/formatter-schema.json` (formatter region) + `_machine/integrations/*.yaml` (integrations region) — future cuts will extend to `_machine/recipes/index.yaml` (recipes matrix in `docs/recipes/index.md`) | `scripts/build-reference-pages.py --check` (regenerates BOTH managed regions and byte-compares; CI wires it into the docs-build job) |
...
| ~~auto-generated reference enumerations inside `docs/reference/*.md` and `docs/integrations/catalogue.md`~~ | ~~E2 Phase 2 — formatter enumeration shipped v1.7-prep~~ + ~~integrations matrix shipped v1.7-prep~~ (see row in the table above; both `docs/reference/formatter.md` and `docs/integrations/catalogue.md` now carry auto-managed regions rendered by `scripts/build-reference-pages.py` from their respective sources of truth, drift-gated in CI). The same framework will extend to `docs/recipes/index.md` (auto matrix from `_machine/recipes/index.yaml`) in a follow-up PR. |
