...
| (managed regions inside `../reference/*.md`, `../integrations/catalogue.md`, and `../recipes/index.md`) | E2 Phase 2 — auto-generated tables that live INSIDE the hand-authored human-readable pages. Currently THREE managed regions: (a) the 83-option formatter enumeration table set in [`docs/reference/formatter.md`](https://github.com/fenre/better_map/blob/main/docs/reference/formatter.md) (grouped by Dashboard Studio tab → in-tab heading, with type, default, enum / range, and help text per option), (b) the at-a-glance integrations matrix + endpoint detail subsection in [`docs/integrations/catalogue.md`](https://github.com/fenre/better_map/blob/main/docs/integrations/catalogue.md) (one row per integration with status, required Splunk app, version min, REST endpoint count, auth model, OT-safety flag, live-tenant test status, and a link back to the source YAML — plus a per-integration `#### <id>` bullet list of every endpoint the viz calls), and (c) the at-a-glance recipes matrix in [`docs/recipes/index.md`](https://github.com/fenre/better_map/blob/main/docs/recipes/index.md) (one row per recipe with source pattern, layer, status, required Splunk apps, expected-fields count, formatter options, OT-safety flag, last-verified date, and a relative link to the recipe markdown). The content lives between `<!-- BEGIN AUTOGEN: <section-id> -->` / `<!-- END AUTOGEN: <section-id> -->` marker pairs; hand-authored narrative outside the markers (including the eight per-integration `## <integration>` prose blocks in catalogue.md, and the `## The recipe contract` / `## Where to read more` sections in recipes/index.md) is preserved verbatim. | `_machine/formatter-schema.json` (formatter region) + `_machine/integrations/*.yaml` (integrations region) + `_machine/recipes/index.yaml` (recipes region) | `scripts/build-reference-pages.py --check` (regenerates ALL THREE managed regions and byte-compares; CI wires it into the docs-build job) |
...
| ~~auto-generated reference enumerations inside `docs/reference/*.md`, `docs/integrations/catalogue.md`, and `docs/recipes/index.md`~~ | ~~E2 Phase 2 — formatter enumeration shipped v1.7-prep~~ + ~~integrations matrix shipped v1.7-prep~~ + ~~recipes matrix shipped v1.7-prep~~ (see row in the table above; all three target pages now carry auto-managed regions rendered by `scripts/build-reference-pages.py` from their respective sources of truth, drift-gated in CI). All three E2 Phase 2 auto-gen targets the original design called out are now shipped. Future work in this lane is per-vertical (e.g. `docs/reference/layers.md` once `_machine/layers/*.yaml` exists; the layer schema is itself blocked behind integrations). |
