...
* **Design:**
  - Ship a manifest with every release: `default/_better_map_manifest.json` listing every file the app owns. On install/upgrade, a `bin/upgrade.py` modular bin script (run via `[restart_postscript]` in `app.conf`) reads the previous-version manifest from `local/_better_map_previous_manifest.json`, computes the set of removed files, and deletes them via REST.
...
* **Prereqs:** None.
* **Risk:** Modular-script execution under `[restart_postscript]` has historically been flaky on Splunk Cloud. Mitigation: fallback to a `setup.xml` action the admin runs once post-upgrade.
* **Accept:** Installing v1.7 on top of v1.6 leaves zero files behind that aren't in the v1.7 manifest; D5 harness asserts this.
...
- [ ] **Dashboard XML/JSON parse check green: every `default/data/ui/views/*.xml` parses, every embedded JSON definition parses**
...
- [ ] v(N-1) → vN upgrade test green (zero orphan files left behind)
