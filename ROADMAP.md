...
> **Status (v1.7.1, 2026-05-19): UX patch SHIPPED — top-left toolbar
> discoverability fix.** First-party dashboard-build feedback: "the
> buttons on the top left of the maps are clickable, but I don't see
> that anything is happening when I do". The draw-tools and measure-
> tool widgets in `appserver/static/visualizations/better_map/src/lib/
> widgets/{drawTools,measure}.js` (rendered top-left of the map when
> `v2DrawTools` / `v2Measure` are enabled in the formatter) were
> behaving correctly — clicking a button activated the corresponding
> drawing/measuring mode — but the only visible feedback was the
> active-state highlight on the button itself. The next interaction
> (clicking the map canvas to drop vertices) was undocumented in the
> UI; tooltips were one-word labels ("Point", "Line", "Polygon",
> "Rectangle", "Circle") that named the mode but not the workflow.
> Result: from a NOC operator's POV the buttons looked clickable-but-
> inert, indistinguishable from a JS error. Root-cause classification:
> discoverability gap, not a functional bug — the widgets were tested
> for click-through, mode state, and shape commitment in v1.6 / v1.7
> but were never validated against "can a new user figure out the
> workflow without docs?". Fix in v1.7.1: each toolbar is now a flex
> COLUMN that stacks a button row (`.better_map-draw__row` /
> `.better_map-measure__row`) on top of an inline status hint
> (`.better_map-draw__hint` / `.better_map-measure__hint`). The hint
> carries `role="status"` + `aria-live="polite"` so screen readers
> announce mode changes. Idle hint: "Pick a shape, then click the
> map" (draw) / "Click the ruler, then click the map to measure"
> (measure). Per-mode hints update on activation, every vertex click,
> commit, and cancel — e.g. polygon flow is "Polygon mode: click the
> map to add the first vertex • Esc to cancel" → "Polygon: 3 vertices
> — click to add more • Double-click or Enter to finish • Esc to
> cancel". Tooltips upgraded from `tool.label` to a dedicated `tool.tip`
> field describing the full workflow ("Draw point — click the map to
> drop markers (stays in point mode for rapid placement)", etc.).
> Measure tool also gained Esc/Enter keyboard handling to match draw-
> tools behaviour (listeners added on `activate`, removed on
> `deactivate`/`clear`). CSS: toolbar `display: flex; flex-direction:
> column;` with `max-width: 220px` (draw) / `260px` (measure); result-
> panel `top` offset bumped to `88px` desktop / `168px` mobile so it
> clears the new hint row. Test coverage: 182 → 202 vitest cases
> (+20). Two new test files —
> `widgets/__tests__/drawTools.test.js` (11 cases: hint element renders
> with correct ARIA, idle hint, button-row wrapping, hint updates on
> mode activation/deactivation, point + rectangle mode-specific hints,
> tooltips describe full workflow, `clear()` / `setEnabled(false)` /
> `setEnabled(true)` lifecycle) and `widgets/__tests__/measure.test.js`
> (9 cases: same coverage matrix adapted for the measure widget).
> Version bumps: `package.json` 1.7.0 → 1.7.1, `app.conf` `[install]
> build` 1700 → 1701, `[launcher] version` + `[id] version` 1.7.0 →
> 1.7.1, `debugHud.js HUD_VERSION` v1.7.0 → v1.7.1. All four sources
> agree per `node scripts/check-version-consistency.js` (PASS).
> `_better_map_manifest.json` regenerated (33 files, 2,603,730 bytes,
> app v1.7.1). All gates green locally: `npm test` (202/202),
> `npm run build` (webpack production, 2.2 MiB minified, zero
> warnings), `python3 scripts/check-manifest.py` (PASS). Zero-
> migration patch: `v2DrawTools` and `v2Measure` still default to
> `false`; dashboards that don't enable them see no visual change.
> Dashboards that do enable them get the inline hints and upgraded
> tooltips automatically with no formatter option changes required.
