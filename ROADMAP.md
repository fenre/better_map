...
* **Phase 2 (partially shipped, see G7 Phase 2 status blocks above):** auto-generated Formatter reference (E2 Phase 2 ✅ via `scripts/build-reference-pages.py` — 82-option enumeration grouped by `(tab, heading)`, drift-gated in CI inside `docs/reference/formatter.md` between BEGIN/END AUTOGEN markers), `llms.txt` emission (G7 Phase 2 ✅); still pending: auto-generated Integration catalogue + per-source recipe matrix sections (same framework, follow-up PRs), Layer reference auto-gen (blocked on `_machine/layers/*.yaml` — de-prioritised behind integrations), privacy-preserving analytics decision, custom domain.
...
> - **What's NOT in Phase 1 (tracked for G7 Phase 2):** ~~`llms.txt`~~ (shipped in v1.7-prep — see Phase 2 status block below), `llms-full.txt` (needs per-page token budget definition + a render pass that strips MkDocs theme chrome), `_machine/layers/<layer-id>.yaml` (independent but de-prioritised behind integrations, where the actual customer questions land), `_machine/openapi-better_map-rest.yaml` (blocked on the REST endpoints it would describe: F1 / G6 / D5 are all v1.8+).
>
...
> - **Still open for G7 Phase 2:** `llms-full.txt` (per-page token-budget contract + theme-chrome stripper, both more involved than the index), `_machine/layers/<layer-id>.yaml` (independent — de-prioritised behind integrations), `_machine/openapi-better_map-rest.yaml` (blocked on F1 / G6 / D5).
>
...
> - **Still open for E2 Phase 2:** auto-generated sections inside `docs/integrations/catalogue.md` (one section per `_machine/integrations/*.yaml`) and `docs/recipes/index.md` (auto matrix from `_machine/recipes/index.yaml`) — both are follow-up PRs against the same `build-reference-pages.py` framework (just new entries in `_regions()`). Privacy-preserving analytics decision + custom domain remain truly Phase 2 work but are independent of the auto-gen track.
...
- [~] **`docs/_machine/` complete: formatter JSON Schema, per-layer YAML, per-integration YAML, recipes index, `agents.md`, OpenAPI for any exposed REST endpoint — each CI-asserted against the implementation it documents** — Phases 1 + 2 partially shipped in v1.7-prep (G7 + E5 + E2): formatter-schema.json (82 options, drift + coverage gates + D3 axe-core a11y), 8 × integrations/*.yaml, agents.md, README.md, recipe-schema.json + recipes/index.yaml (E5 Phase 1 — drift-gated, three starter recipes), llms.txt + build-llms-txt.py (G7 Phase 2 — drift-gated), build-reference-pages.py (E2 Phase 2 — managed-region regenerator for `docs/reference/formatter.md`, drift-gated; framework extends to integrations catalogue + recipe index in follow-up PRs); still open: per-layer YAML (layers/*.yaml), llms-full.txt, OpenAPI (blocked on REST endpoints in v1.8+)
