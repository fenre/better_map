...
> - **Still open for E2 Phase 2:** ~~auto-generated sections inside `docs/integrations/catalogue.md`~~ (shipped v1.7-prep — see "E2 Phase 2 SHIPPED (auto-gen integrations matrix)" status block below) and `docs/recipes/index.md` (auto matrix from `_machine/recipes/index.yaml`) — the remaining cut is a follow-up PR against the same `build-reference-pages.py` framework (just one more entry in `_regions()`). Privacy-preserving analytics decision + custom domain remain truly Phase 2 work but are independent of the auto-gen track.
>
...
> - **Still open for E2 Phase 2:** auto-generated matrix inside `docs/recipes/index.md` (one row per recipe in `_machine/recipes/index.yaml`) — a follow-up PR adding one more `ManagedRegion` to `_regions()`. Privacy-preserving analytics decision + custom domain remain independent of the auto-gen track.
